...
contract('TraderPool', (accounts) => {
    ...
    // todo should happen before unittests in before
    it('should init Uniswap pool for basicToken/anotherToken', async () => {
        ...
        let riskToken1Amount = toBN('99660').mul(decimals);  // todo discuss amount
        let riskToken2Amount = toBN('99660').mul(decimals);
        ...
        await basicToken.approve.sendTransaction(uniswapRouterAddress, basicTokenAmount);
        await riskToken1.approve.sendTransaction(uniswapRouterAddress, riskToken1Amount);
        await uniswapRouter.addLiquidity.sendTransaction(
            basicToken.address, riskToken1.address,
            basicTokenAmount, riskToken1Amount,
            basicTokenAmount, riskToken1Amount,
            accounts[0],
            new Date().getTime() + (2 * 24 * 60 * 60 * 1000)
        );
        ...
        await basicToken.approve.sendTransaction(uniswapRouterAddress, basicTokenAmount);
        await riskToken2.approve.sendTransaction(uniswapRouterAddress, riskToken2Amount);
        await uniswapRouter.addLiquidity.sendTransaction(
            basicToken.address, riskToken2.address,
            basicTokenAmount, riskToken2Amount,
            basicTokenAmount, riskToken2Amount,
            accounts[0],
            new Date().getTime() + (2 * 24 * 60 * 60 * 1000)
        );
